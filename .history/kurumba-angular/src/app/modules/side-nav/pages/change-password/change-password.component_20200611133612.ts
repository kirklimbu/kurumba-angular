...
export class ChangePasswordComponent implements OnInit {
  ...
  getOldPassErrorMessage() {
    return this.changePasswordForm.controls['oldPass'].hasError('required') ? 'Student name is required.' :
      this.changePasswordForm.controls['name'].hasError('maxLength') ? 'Invalid name.' :
        this.changePasswordForm.controls['name'].hasError('minlength') ? 'Name must be atleast of 2 characters.' :
          '';
  } getNewPassErrorMessage() {
    return this.changePasswordForm.controls['name'].hasError('required') ? 'Student name is required.' :
      this.changePasswordForm.controls['name'].hasError('maxLength') ? 'Invalid name.' :
        this.changePasswordForm.controls['name'].hasError('minlength') ? 'Name must be atleast of 2 characters.' :
          '';
