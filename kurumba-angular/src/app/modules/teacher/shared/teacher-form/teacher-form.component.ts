...
export class TeacherFormComponent implements OnInit {
  ...
  onSave(mode) {
    ...
    if (this.teacherForm.valid && this.formSubmitted) {
      this.spinner.show();
      this.teacherService
        .teacherService(this.teacherForm.value)
        .pipe(finalize(() => this.spinner.hide()))
        .subscribe(
          (res) => {
            this.toastr.success("New Teacher SuccessFully Created.");
          ...
          (err) => {
            ...
            err = err.error.message
              ? this.toastr.error(err.error.message)
              : this.toastr.error("Error adding new teacher.");
          }
        );
    }
  }
  ...
  // error message block
  getNameErrorMessage() {
    return this.teacherForm.controls.name.hasError("required")
      ? "Teacher name is required."
      : this.teacherForm.controls.name.hasError("maxLength")
      ? "Invalid name."
      : this.teacherForm.controls.name.hasError("minlength")
      ? "Name must be atleast of 2 characters."
      : "";
  }

  getaddressErrorMessage() {
    return this.teacherForm.controls.address.hasError("required")
      ? "Teacher address is required."
      : this.teacherForm.controls.address.hasError("maxLength")
      ? "Invalid address."
      : this.teacherForm.controls.address.hasError("minlength")
      ? "Address must be atleast of 2 characters."
      : "";
  }
  getphoneErrorMessage() {
    return this.teacherForm.controls.phoneNo.hasError("required")
      ? "Mobile num is required."
      : this.teacherForm.controls.phoneNo.hasError("maxLength")
      ? "Invalid phone num."
      : this.teacherForm.controls.phoneNo.hasError("minlength")
      ? "Invalid phone num."
      : "";
  }

  getqualificationErrorMessage() {
    return this.teacherForm.controls.qualification.hasError("required")
      ? "Teacher  roll num is required."
      : this.teacherForm.controls.qualification.hasError("maxLength")
      ? "Invalid roll num."
      : this.teacherForm.controls.qualification.hasError("minlength")
      ? "Roll num must be atleast of 1 character."
      : "";
  }

  getDobErrorMessage() {
    return this.teacherForm.controls.dob.hasError("required")
      ? "Teacher dob is required."
      : "";
  }

  getSubjectErrorMessage() {
    return this.teacherForm.controls["subjectList"].hasError("required")
      ? "Subject is required."
      : "";
  }
